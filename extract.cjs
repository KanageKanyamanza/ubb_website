...
function processDir(dir) {
    ...
    for (const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            if (file !== 'ui') { // Skip basic UI components like buttons to reduce noise
                processDir(fullPath);
            }
            ...
            for (let line of lines) {
                ...
                if (line.includes('`')) continue;
                if (line.includes('px-') || line.includes('py-') || line.includes('bg-') || line.includes('text-')) continue; // Classes CSS résiduelles
